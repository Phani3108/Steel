...
Runtimes (steel-engine's LangGraph compiler, the miniloop, anything future) compile
...
class AutonomyLevel(IntEnum):
    ...
    steel-dyno scorecards — an agent is promoted when it proves itself, never by edit."""
...
class HITLGate(BaseModel):
    """A named checkpoint where the run pauses for human approval (steel-brakes)."""
...
class Mandate(BaseModel):
    ...
    bounded delegation). Violations are blocked by steel-governor and audited, not retried."""
...
class AgentManifest(BaseModel):
    api_version: Literal["steel/v1"] = "steel/v1"
    name: str
    description: str
