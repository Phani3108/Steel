...
of JAI. Bring any OpenAI-compatible gateway client with a
...
agent = compile_manifest(
    manifest,
    gateway=GatewayClient(),       # JAI_MOCK=1 (default) -> keyless mock completions
...
result = agent.run(ctx, "Hello, JAI.")          # thread_id=... for resumable runs
...
(default `postgresql://jai:jai@localhost:5433/jai`).

## Env

| Variable | Default | Used for |
|---|---|---|
| `POSTGRES_URL` | `postgresql://jai:jai@localhost:5433/jai` | LangGraph checkpointer (optional) |
| `LITELLM_BASE_URL` | `http://localhost:4000` | via jai-gateway |
| `LITELLM_MASTER_KEY` | `sk-jai-master-dev` | via jai-gateway |
| `JAI_MOCK` | `1` | keyless mock completions via jai-gateway |
