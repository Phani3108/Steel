...
same cost-control surface with zero JAI coupling beyond the `RunContext` envelope.
...
| **Keyless mock mode** | `JAI_MOCK=1` (the default) sends LiteLLM's `mock_response` so the whole platform runs with no provider keys; cost is 0.0 by definition |
...
client = GatewayClient()  # env: LITELLM_BASE_URL, LITELLM_MASTER_KEY, JAI_MOCK
...
```python
from jai_gateway import provision_virtual_key
...
| `LITELLM_MASTER_KEY` | `sk-jai-master-dev` | Auth against the proxy |
| `JAI_MOCK` | `1` | Keyless mock mode; set `0` for live provider calls |
